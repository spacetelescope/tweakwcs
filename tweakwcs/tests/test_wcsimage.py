...
def test_wcsimcat_wrong_tpwcs_type(mock_fits_wcs):
    ...
    assert (
        e.value.args[0] == "Unsupported 'corrector' type. 'corrector' "
...
def test_wcsimcat_wcs_transforms_roundtrip(mock_fits_wcs):
    ...
    assert np.allclose(w.world_to_det(*w.det_to_world(x, y)), (x, y), rtol=0, atol=1.0e-5)

    assert np.allclose(w.tanp_to_det(*w.det_to_tanp(x, y)), (x, y), rtol=0, atol=1.0e-5)
...
def test_wcsimcat_intersections(mock_fits_wcs, rect_imcat):
    ...
    assert np.allclose(
        rect_imcat.intersection_area(rect_imcat), 2.9904967391303217e-12, atol=0.0, rtol=5.0e-4
    )
...
def test_wcsimcat_calc_chip_bounding_polygon_custom_stepsize(mock_fits_wcs, rect_imcat):
...
def test_wcsimcat_bb_radec(mock_fits_wcs, rect_imcat):
    ra1, dec1 = rect_imcat.det_to_world(rect_imcat.catalog["x"], rect_imcat.catalog["y"])
...
def test_wcsgroupcat_init(mock_fits_wcs, rect_imcat):
    ...
    with pytest.raises(TypeError) as e:
        WCSGroupCatalog([1])
    assert (
        e.value.args[0] == "Each element of the 'images' parameter "
    ...
    with pytest.raises(TypeError) as e:
        WCSGroupCatalog(1)
    assert (
        e.value.args[0] == "Parameter 'images' must be either a single "
    ...
    assert e.value.args[0] == "Each input WCS image catalog must have a valid catalog."
...
def test_wcsgroupcat_intersections(mock_fits_wcs, rect_imcat):
    ...
    assert np.allclose(g.intersection_area(g), 2.9904967391303217e-12, atol=0.0, rtol=5.0e-4)


def test_wcsgroupcat_guarded_intersection_area(mock_fits_wcs, rect_imcat):
    g = WCSGroupCatalog(rect_imcat)
    assert np.allclose(
        g._guarded_intersection_area(g)[0], 2.9904967391303217e-12, atol=0.0, rtol=5.0e-4
    )
...
def test_wcsgroupcat_empty_cat(mock_fits_wcs, rect_imcat):
    ...
    ra, dec = mock_fits_wcs.all_pix2world(rect_imcat.catalog["x"], rect_imcat.catalog["y"], 0)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            np.array([[0.0, 0.0], [0.3, 0.2]]),
            np.array([[-0.5, -0.5], [0.8, -0.5], [0.8, 0.7], [-0.5, 0.7], [-0.5, -0.5]]),
        ),
        (
        ...
        (
            np.array([[1.1, 1.2]]),
            np.array([[0.6, 0.7], [1.6, 0.7], [1.6, 1.7], [0.6, 1.7], [0.6, 0.7]]),
        ),
    ],
)
def test_wcsimage_tiny_cat(mock_fits_wcs, xy, expected_rect):
    imcat = Table([xy[:, 0], xy[:, 1]], names=("x", "y"))
    corr = FITSWCSCorrector(mock_fits_wcs)
    ...
    ra, dec = tuple(w.polygon.to_radec())[0]
    ra = sorted(map(float, ra))
    dec = sorted(map(float, dec))

    assert np.allclose(ra, ra_expected, atol=0.0, rtol=1.0e-12)
    assert np.allclose(dec, dec_expected, atol=0.0, rtol=1.0e-12)
...
def test_wcsgroupcat_create_group_catalog(mock_fits_wcs, rect_imcat):
    ...
    with pytest.raises(KeyError) as e:
        ...
        pytest.fail("Expected a KeyError due to missing 'weight' column in one of the catalogs.")
    assert (
        e.value.args[0] == "Non-empty catalogs in a group must all "
...
def test_wcsgroupcat_bb_policy(rect_imcat):
    ...
    assert e.value.args[0].startswith("'bb_policy' must be either 'auto', 'exact'")
...
def test_wcsgroupcat_match2ref(mock_fits_wcs, rect_imcat):
    ...
    assert e.value.args[0] == "'calc_tanp_xy()' should have been run prior to match2ref()"
...
def test_wcsrefcat_init_check_catalog(mock_fits_wcs):
    ...
    assert e.value.args[0] == "Reference catalogs *must* contain *both* 'RA' and 'DEC' columns."

    r.expand_catalog(Table([[1], [1]], names=("RA", "DEC")))


def test_wcsrefcat_intersections(mock_fits_wcs, rect_imcat):
    ra, dec = mock_fits_wcs.all_pix2world(
        10 * rect_imcat.catalog["x"], 10 * rect_imcat.catalog["y"], 0
    )
    refcat = Table([ra, dec], names=("RA", "DEC"))
    ref = RefCatalog(refcat)
    ref.calc_tanp_xy(rect_imcat.corrector)
...
def test_wcsrefcat_convex_hull(mock_fits_wcs):
    ...
    assert convex_hull([], []) == ([], [])
    assert all(
        isinstance(x, np.ndarray) and x.size == 0 for x in convex_hull(np.array([]), np.array([]))
    )
    ...
    xc, yc = convex_hull(np.array(x + [3, 6, 7]), np.array(y + [5, 1, 8]), lambda x, y: (x, y))
    assert set(xc) == set(x) and set(yc) == set(y)
